...
import { UploadSimple, CaretLeft, Trash, TextH, TextT, Image as ImageIcon, ArrowUp, ArrowDown, Spinner, VideoCamera, TextB, TextItalic, LinkSimple, ListBullets, ListNumbers, Quotes } from '@phosphor-icons/react';
...
export default function EditCaseStudy() {
    ...
    useEffect(() => {
        const fetchStudy = async () => {
            ...
            if (error || !data) {
                setFetchError(error?.message || 'Case study not found.');
                setIsLoading(false);
                return;
            }
            ...
            setFormData({
                ...
                logo_text: study.logo_text,
                logo_bg: study.logo_bg,
                testimonial_text: study.testimonial_text,
                testimonial_author: study.testimonial_author,
                testimonial_stars: study.testimonial_stars,
                testimonial_avatar: study.testimonial_avatar,
                is_featured: study.is_featured,
                year: study.year || '',
                timeline: study.timeline || '',
            });
            setExistingImageUrl(study.image_url);
            ...
            // Parse scope of work
            if (study.scope_of_work && Array.isArray(study.scope_of_work)) {
                setScopeRaw(study.scope_of_work.join(', '));
            }

            // Parse content blocks
            const content = study.content as unknown as RichContentBlock[];
    ...
    const handleBlockImageChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            updateBlock(index, e.target.files[0].name);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (imageFile) {
                ...
                if (uploadError) { alert('Upload error: ' + uploadError.message); throw uploadError; }
            ...
            if (logoFile) {
                ...
                if (logoErr) { alert('Logo upload error: ' + logoErr.message); throw logoErr; }
            ...
            for (let i = 0; i < contentBlocks.length; i++) {
                ...
                if (block.type === 'image' && blockImageFiles[i]) {
                    ...
                    if (err) { alert('Block media error: ' + err.message); throw err; }

                    const { data: { publicUrl } } = supabase.storage.from('images').getPublicUrl(path);
                    processedBlocks.push({ type: block.type, value: publicUrl });
                } else if (block.type === 'video' && blockImageFiles[i]) {
                    const file = blockImageFiles[i];
                    const ext = file.name.split('.').pop();
                    const path = `case-studies/${Date.now()}-block-${i}.${ext}`;
                    ...
                    const { data: { publicUrl } } = supabase.storage.from('images').getPublicUrl(path);
                    processedBlocks.push({ type: 'video', value: publicUrl });
                } else {
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center py-24">
                ...
                <span className="ml-3 text-zinc-500 text-sm">Loading case study&hellip;</span>
            </div>
        );
    }
        ...
        return (
            <div className="font-sans pb-12">
                ...
                <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
                    <p className="text-red-600 font-semibold text-lg mb-2">Failed to load case study</p>
    ...
    return (
        <div className="font-sans pb-12">
            ...
            <h1 className="text-3xl font-bold text-ink mb-8">Edit: {formData.title}</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input required type="text" className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink font-mono text-sm"
                        ...
                        <div>
                            ...
                            <input type="text" placeholder="e.g. 2024" className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink"
                                value={formData.year} onChange={e => setFormData({ ...formData, year: e.target.value })} />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-zinc-700 mb-2">Timeline</label>
                            <input type="text" placeholder="e.g. 12 weeks" className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink"
                    ...
                    <div>
                        ...
                        <textarea required rows={3} className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink resize-none"
                ...
                <div className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
                        ...
                        {contentBlocks.map((block, index) => (
                            <div key={index} className="border border-zinc-200 rounded-2xl p-4 bg-zinc-50/50">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-1">
                                        <button type="button" onClick={() => moveBlock(index, 'up')} disabled={index === 0}
                                            className="p-1.5 rounded-lg hover:bg-zinc-200 text-zinc-500 disabled:opacity-30 transition-colors"><ArrowUp size={14} /></button>
                                        <button type="button" onClick={() => moveBlock(index, 'down')} disabled={index === contentBlocks.length - 1}
                                            className="p-1.5 rounded-lg hover:bg-zinc-200 text-zinc-500 disabled:opacity-30 transition-colors"><ArrowDown size={14} /></button>
                                        <button type="button" onClick={() => removeBlock(index)}
                                            className="p-1.5 rounded-lg hover:bg-red-50 text-zinc-400 hover:text-red-500 transition-colors ml-2"><Trash size={14} /></button>
                                    </div>
                                </div>
                                {block.type === 'heading' && (
                                ...
                                {(block.type === 'image' || block.type === 'video') && (
                                    <div>
                                        {block.value && !blockImageFiles[index] && (
                                            block.type === 'video'
                                                ? <video src={block.value} className="w-full h-40 object-cover rounded-xl mb-3 border border-zinc-200" muted />
                                                : <img src={block.value} alt="" className="w-full h-40 object-cover rounded-xl mb-3 border border-zinc-200" />
                                        )}
                                        <input type="file" accept={block.type === 'video' ? 'video/*' : 'image/*,video/*'} id={`block-media-${index}`} className="hidden"
                                        ...
                                        <label htmlFor={`block-media-${index}`}
                                            ...
                                            <span className="text-sm text-zinc-600">{blockImageFiles[index]?.name || `Replace ${block.type}`}</span>
                                        </label>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="flex items-center gap-3 pt-2">
                        <button type="button" onClick={() => addBlock('heading')} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 text-sm font-semibold text-zinc-600 hover:bg-zinc-50 hover:border-zinc-300 transition-all">
                            <TextH size={16} /> Heading
                        </button>
                        <button type="button" onClick={() => addBlock('text')} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 text-sm font-semibold text-zinc-600 hover:bg-zinc-50 hover:border-zinc-300 transition-all">
                            <TextT size={16} /> Paragraph
                        </button>
                        <button type="button" onClick={() => addBlock('image')} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 text-sm font-semibold text-zinc-600 hover:bg-zinc-50 hover:border-zinc-300 transition-all">
                            <ImageIcon size={16} /> Image
                        </button>
                        <button type="button" onClick={() => addBlock('video')} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 text-sm font-semibold text-zinc-600 hover:bg-zinc-50 hover:border-zinc-300 transition-all">
                ...
                <div className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
                    ...
                    {existingImageUrl && !imageFile && (
                        (() => {
                            const lower = existingImageUrl.toLowerCase();
                            const isVid = ['.mp4', '.webm', '.mov', '.ogg'].some(e => lower.includes(e));
                            return isVid
                                ? <video src={existingImageUrl} className="w-full max-h-64 object-cover rounded-2xl border border-zinc-200 mb-4" muted autoPlay loop playsInline />
                                : <img src={existingImageUrl} alt="Current hero" className="w-full max-h-64 object-cover rounded-2xl border border-zinc-200 mb-4" />;
                        })()
                    )}
                    <div>
                        <label className="block text-sm font-semibold text-zinc-700 mb-2">Replace Hero Media</label>
                        <div className="border-2 border-dashed border-zinc-200 rounded-2xl p-6 text-center hover:bg-zinc-50 transition-colors">
                            <input type="file" id="image-upload" className="hidden" accept="image/*,video/*"
                                onChange={(e) => { if (e.target.files?.[0]) setImageFile(e.target.files[0]); }} />
                            <label htmlFor="image-upload" className="cursor-pointer flex flex-col items-center">
                                <UploadSimple size={28} className="text-zinc-400 mb-2" />
                                <span className="text-ink font-semibold">{imageFile ? imageFile.name : 'Click to upload new media'}</span>
                    ...
                    <div>
                        ...
                        {existingLogoUrl && !logoFile && (
                            <img src={existingLogoUrl} alt="Current logo" className="w-20 h-20 object-contain rounded-xl border border-zinc-200 mb-3" />
                        )}
                        <div className="border-2 border-dashed border-zinc-200 rounded-2xl p-6 text-center hover:bg-zinc-50 transition-colors">
                            ...
                            <label htmlFor="logo-upload" className="cursor-pointer flex flex-col items-center">
                                ...
                                <span className="text-ink font-semibold text-sm">{logoFile ? logoFile.name : 'Upload new logo'}</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-semibold text-zinc-700 mb-2">Logo Text</label>
                            <input required type="text" maxLength={2} className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink"
                ...
                <div className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
                    ...
                    <textarea rows={3} className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink resize-none"
                        value={formData.testimonial_text} onChange={e => setFormData({ ...formData, testimonial_text: e.target.value })} />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-semibold text-zinc-700 mb-2">Author</label>
                            <input type="text" className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink"
                        ...
                        <div>
                            ...
                            <input type="url" className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-ink"
                ...
                <div className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <input type="checkbox" id="featured" className="w-5 h-5 rounded border-zinc-300 text-ink focus:ring-ink"
